...
class UrImpedanceController(threading.Thread):
    def __init__(
        ...
        self.reset_height = np.array([getattr(config, "RESET_HEIGHT", 0.1)], dtype=np.float32)
    ...
    async def start_ur_interfaces(self, gripper=True):
        ...
        if gripper:
            gripper_comm = str(getattr(self.config, "GRIPPER_COMMUNICATION", "usb")).lower()
            ...
            await self.robotiq_gripper.activate(
                auto_calibrate=bool(getattr(self.config, "GRIPPER_AUTO_CALIBRATE", True))
            )
        if self.verbose:
    ...
    def _truncate_check(self):
        ...
        if force_limit > 0.0 and np.linalg.norm(self.curr_force_lowpass[:3]) > force_limit:
            self._is_truncated.set()
        else:
    ...
    async def _go_to_reset_pose(self):
        ...
        reset_gripper_action = str(getattr(self.config, "RESET_GRIPPER_ACTION", "release")).lower()
        if self.robotiq_gripper and reset_gripper_action != "none":
            if reset_gripper_action in ("release", "open"):
                ...
                raise ValueError(f"Unknown RESET_GRIPPER_ACTION: {reset_gripper_action}")
            time.sleep(0.01)
    ...
    async def run_async(self):
        ...
        try:
            ...
            release_on_close = bool(getattr(self.config, "RELEASE_GRIPPER_ON_CLOSE", False))
            if self.robotiq_gripper:
                if release_on_close:
                    await self.send_gripper_command(force_release=True)
                    time.sleep(0.05)
            ...
            if bool(getattr(self.config, "MOVE_HOME_ON_CLOSE", False)):
                reset_Q = np.asarray(getattr(self.config, "CLOSE_Q", self.reset_Q), dtype=np.float64).reshape(6)
