...
class UR5Env(gym.Env):
    def __init__(
        ...
        self.video_record_dir = os.getenv("VIDEO_RECORD_DIR", "./videos")
        self.video_record_fps = int(os.getenv("VIDEO_RECORD_FPS", str(hz)))
        self.recording_frames = OrderedDict()
    ...
    def save_video_recording(self):
        try:
            if any(len(frames) for frames in self.recording_frames.values()):
                os.makedirs(self.video_record_dir, exist_ok=True)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            for camera_key, frames in self.recording_frames.items():
                if not frames:
                    continue
                video_path = os.path.join(
                    self.video_record_dir, f"{camera_key}_{timestamp}.mp4"
                )
    ...
    def get_image(self) -> Dict[str, np.ndarray]:
        ...
        if self.save_video:
            for key, image in display_images.items():
                if "full" in key:
                    self.recording_frames.setdefault(key.replace("_full", ""), []).append(
