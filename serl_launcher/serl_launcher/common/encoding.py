...
class EncodingWrapper(nn.Module):
    ...
    def __call__(
        ...
        # Causal mask batches contain regular and synthetic rows together. Synthetic
        # rows bypass pixel encoding with a counterfactually augmented latent.
        if "encoded_features" in observations:
            if "is_encoded" not in observations:
                raise ValueError("encoded_features requires an is_encoded mask")
            encoded_mask = observations["is_encoded"]
            while encoded_mask.ndim < encoded.ndim:
                encoded_mask = encoded_mask[..., None]
            encoded = jnp.where(encoded_mask, observations["encoded_features"], encoded)
